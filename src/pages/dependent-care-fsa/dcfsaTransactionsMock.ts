import type { FsaTransactionRow } from "@/pages/fsa-account/fsaTransactionsMock";

/** Mock Dependent Care FSA transactions — matches Transactions table reference design. */
export const dcfsaTransactionsData: FsaTransactionRow[] = [
  {
    id: "dc1",
    date: "03/01/2026",
    status: "Complete",
    description: "Payroll Deduction",
    planYear: "01/01/2026 - 12/31/2026",
    amount: "$192.31",
  },
  {
    id: "dc2",
    date: "02/28/2026",
    status: "Complete",
    description: "Sunshine Daycare",
    planYear: "01/01/2026 - 12/31/2026",
    amount: "-$576.93",
    amountIsNegative: true,
  },
  {
    id: "dc3",
    date: "02/15/2026",
    status: "Complete",
    description: "Payroll Deduction",
    planYear: "01/01/2026 - 12/31/2026",
    amount: "$192.31",
  },
  {
    id: "dc4",
    date: "01/30/2026",
    status: "Complete",
    description: "Payroll Deduction",
    planYear: "01/01/2026 - 12/31/2026",
    amount: "$192.31",
  },
  {
    id: "dc5",
    date: "01/15/2026",
    status: "Complete",
    description: "Payroll Deduction",
    planYear: "01/01/2026 - 12/31/2026",
    amount: "$192.31",
  },
  {
    id: "dc6",
    date: "01/01/2026",
    status: "Complete",
    description: "Sunshine Daycare",
    planYear: "01/01/2026 - 12/31/2026",
    amount: "-$576.93",
    amountIsNegative: true,
  },
  {
    id: "dc7",
    date: "01/01/2026",
    status: "Complete",
    description: "Payroll Deduction",
    planYear: "01/01/2026 - 12/31/2026",
    amount: "$192.31",
  },
  {
    id: "dc8",
    date: "01/01/2026",
    status: "Complete",
    description: "Payroll Deduction",
    planYear: "01/01/2026 - 12/31/2026",
    amount: "$192.31",
  },
  {
    id: "dc9",
    date: "12/17/2025",
    status: "Complete",
    description: "Sunshine Daycare",
    planYear: "01/01/2025 - 12/31/2025",
    amount: "-$67.25",
    amountIsNegative: true,
  },
  {
    id: "dc10",
    date: "12/10/2025",
    status: "Complete",
    description: "Sunshine Daycare",
    planYear: "01/01/2025 - 12/31/2025",
    amount: "-$1,027.55",
    amountIsNegative: true,
  },
];
