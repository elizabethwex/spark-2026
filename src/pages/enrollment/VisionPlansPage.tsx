...
const STORAGE_KEY = "enrollment.vision-plans.v1";
const DEPENDENTS_STORAGE_KEY = "enrollment.dependents.v1";

type VisionPlansPageProps = {
...
export default function VisionPlansPage({
  currentStepId,
  onStepChange,
  onBack,
  onNext,
  onCancel,
}: VisionPlansPageProps) {
  const [dependents, setDependents] = React.useState<DependentFromStorage[]>([]);
  const [state, setState] = React.useState<VisionPlansState>({
    enrollees: ["myself"],
    selectedPlanId: null,
  });
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      if (raw) {
        const parsed = JSON.parse(raw) as VisionPlansState;
      ...
      <div className="pt-14 flex justify-center">
        <div className="w-[min(900px,92vw)]">
          ...
          <p className="mt-2 text-[14px] leading-6 tracking-[-0.084px] text-muted-foreground">
            Select your vision coverage and choose who to enroll.
          </p>
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-3">
            {plans.map((plan) => {
              ...
              const details = plan.isWaive
                ...
                : plan.id === "premium"
                  ? [
                      { label: "Exam Coverage", value: "$10 copay" },
                      { label: "Frame Allowance", value: "$200" },
                      { label: "Contact Lens Allowance", value: "$150" },
                      { label: "Benefit Frequency", value: "Annual" },
                    ]
                  : [
                      { label: "Exam Coverage", value: "$25 copay" },
                      { label: "Frame Allowance", value: "$130" },
                      { label: "Contact Lens Allowance", value: "$100" },
                      { label: "Benefit Frequency", value: "Annual" },
                    ];
