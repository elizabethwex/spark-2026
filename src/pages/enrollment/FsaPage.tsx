...
import { loadFsaState, saveFsaState } from "@/lib/fsaStorage";
...
export default function FsaPage({ currentStepId, onStepChange, onBack, onNext, onCancel }: FsaPageProps) {
  const initial = React.useMemo(() => loadFsaState(), []);
  ...
  const taxSavingsCents = Math.round(clampedElectionCents * TAX_RATE);
  const perPayDeductionCents =
    PAY_PERIODS_PER_YEAR > 0 ? Math.round(clampedElectionCents / PAY_PERIODS_PER_YEAR) : 0;
          ...
          <div className="absolute right-0 top-0 w-[344px]">
            <div className="bg-white rounded-[12px] border border-border shadow-[0px_4px_10px_0px_rgba(2,13,36,0.15),0px_0px_1px_0px_rgba(2,13,36,0.3)] p-4">
              ...
              <div className="flex items-center justify-between text-[14px] leading-6 tracking-[0.28px] text-foreground">
                ...
                <div className="text-right">{formatCurrencyFromCents(taxSavingsCents)}</div>
              </div>
              
              <div className="flex items-center justify-between text-[14px] leading-6 tracking-[0.28px] text-foreground">
                <div>Estimated pay period deduction:</div>
                <div className="text-right">{formatCurrencyFromCents(perPayDeductionCents)}</div>
              </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <label className="flex items-center gap-2 text-[14px] leading-6 text-foreground">
                <WexCheckbox
                  ...
                  aria-label="I have read and understand the FSA rules"
                />
                <span>
