...
const STORAGE_KEY = "enrollment.medical-plans.v1";
const DEPENDENTS_STORAGE_KEY = "enrollment.dependents.v1";

type MedicalPlansPageProps = {
...
export default function MedicalPlansPage({
  ...
  onNext,
  onCancel,
}: MedicalPlansPageProps) {
  const [dependents, setDependents] = React.useState<DependentFromStorage[]>([]);
  const [state, setState] = React.useState<MedicalPlansState>({
    enrollees: ["myself"],
    selectedPlanId: null,
  });
  ...
  const coverageLabel = (() => {
    ...
    if (selectedDependentsCount === 1) return "Employee + 1 dependent";
    return "Family";
  })();
  return (
    <EnrollmentLayout
      ...
      <div className="pt-14 flex justify-center">
        <div className="w-[min(900px,92vw)]">
          ...
          <p className="mt-2 text-[14px] leading-6 tracking-[-0.084px] text-muted-foreground">
            Select your medical coverage and choose who to enroll.
          </p>
